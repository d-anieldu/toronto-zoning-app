...
const FIELD_CONFIG: Record<string, { label: string; fmt: (v: any) => string; higher?: boolean }> = {
  ...
  min_lot_area_sqm: { label: "Min Lot Area (m²)", fmt: (v) => (v != null ? `${Number(v).toLocaleString()}` : "—"), higher: true },
  lot_area_sqm: { label: "Parcel Size (m²)", fmt: (v) => (v != null ? `${Number(v).toLocaleString()}` : "—"), higher: true },
